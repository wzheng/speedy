...
Figure 3 shows a linear relationship between the two. As mentioned above, the security of the Whanau protocol can support
up to $O(n/logn)$ attack edges, where $n$ is the number of honest nodes in the network. In the experiment, we construct
a 100 node network with $50$ honest and $50$ sybil nodes, each region is well connected with each other.
We vary the number of edges (attack edges) between the two regions and observe the effect of the
number of Sybil nodes that end up in the Paxos clusters. In the experiment, the $n$ is 50, so the $O(n/logn)$ cut off is around 12 attack edges.
As seen from the graph, the percentage of Sybil majority Paxos clusters is near zero for 12 attack edges.

\begin{figure}[h]
\centering
\includegraphics[width=0.8\columnwidth]{sybilcluster-3}
...
The honest region is well connected and the sybil region is well connected.}
